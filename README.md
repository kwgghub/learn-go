...
✨ 交互式编程练习 · 🔴 实时代码运行 · ✅ 自动判分 · 🎯 渐进式解锁

[快速开始](#快速开始) · [功能特性](#功能特性) · [课程大纲](#课程大纲) · [技术栈](#技术栈)
...
类似 [learn-py.org](https://learn-py.org/) 的 **交互式 Go 语言学习网站**。在浏览器里写代码、当场运行、自动判分、做对解锁下一题。

专为零基础小白设计，从 Hello World 到并发编程，一题一题练，代码当场跑，判分秒回。

## ✨ 功能特性

### 🎮 交互式学习体验
- **Monaco 代码编辑器** — 专业级代码编辑器，语法高亮、自动补全
- **实时运行代码** — 一键运行，沙箱环境执行 `go run`
...
|------|------|------|
| GET | `/api/health` | 健康检查 |
| GET | `/api/catalog` | 获取课程目录 |
| GET | `/api/chapters/{slug}` | 获取章节详情 |
| POST | `/api/run` | 运行代码 |
...
- [ ] Docker / gVisor 安全沙箱（生产环境必备）
- [ ] go test 判题 — 支持函数级单元测试
- [ ] 用户系统 — 登录、云端同步进度
- [ ] 更多章节 — HTTP 服务、数据库、测试、部署
- [ ] 代码自动补全 — LSP 智能提示
- [ ] 移动端适配优化
- [ ] 多语言支持
